...
from my_self_avoiding_walk import SAW
...
# entries for the number of steps of random walk 
...
#entry for the numbers of self-avoiding walk attempts
# if walker comes to the previously visited site the walk is rejected
#new walk starts
...
def dialog():
    #dialog function in tkinter in the loop
    numc = steps_entry.get()
	# check if numc is a number (in string format)
    if not(numc.isdigit()):
        message.showinfo('Greetings','# steps: not a number. will run default 10 steps ')
        numc = '10'
    num_steps=int(numc)
    z=tkvar.get()  # type of random walk
    #print('steps: '+numc + '  walktype: '+z+'\n')
    if (z=='NR RANDOM WALK'):
	#non-returning random walk in 2 dimensions 
        rw=MyNRRandomWalk(num_steps+1) # without +1 it makes n-1 steps
        rw.fill_walk()
        xwalk=rw.get_x()
        #print(xwalk)
        ...
        if not(num_samples_c.isdigit()):
            message.showinfo('Greetings',' # samples: not a number. will run default 10 steps ')
            num_samples = '10'
        ...
        mysaw = SAW(num_samples,num_steps+1) # see num_steps in line 39
